...
async function main() {
  ...
  console.log('Launching browser to isolate the bottle on pure white with clean feathered edges...');
  ...
  const result = await page.evaluate(async () => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Find Y bounds (ignore shadow/background at top/bottom, threshold = 35)
        for (let y = 0; y < height; y++) {
          const bg = rowBg[y];
        ...
        // We use an aggressive threshold range [28, 38] to eliminate the base shadow and grey background completely
        const minThreshold = 28;
        const maxThreshold = 38;
  ...
  console.log(`Successfully wrote clean Moringa Oil image to: ${destPath}`);
