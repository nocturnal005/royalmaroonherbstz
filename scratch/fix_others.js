...
async function fixMultiple() {
  ...
  for (const file of files) {
    ...
    const rawDataUri = `data:image/jpeg;base64,${rawBuffer.toString('base64')}`;
    ...
    const result = await page.evaluate(async (uri) => {
      return new Promise((resolve) => {
        ...
        img.onload = () => {
          ...
          const OUTPUT = 1600; const TARGET_FILL = 0.78;
          ...
          finalCanvas.width = OUTPUT; finalCanvas.height = OUTPUT;
          const finalCtx = finalCanvas.getContext('2d');
          finalCtx.fillStyle = '#ffffff'; finalCtx.fillRect(0, 0, OUTPUT, OUTPUT);

          const destX = Math.round((OUTPUT - renderW) / 2);
          const destY = Math.round((OUTPUT - renderH) / 2);
          finalCtx.drawImage(tempCanvas, minX, minY, cropW, cropH, destX, destY, renderW, renderH);
    ...
    fs.writeFileSync(destPath, Buffer.from(result.replace(/^data:image\/jpeg;base64,/, ''), 'base64'));
    console.log(`Fixed ${file}`);
  }
  await browser.close();
}

fixMultiple().catch(console.error);
