...
async function main() {
  ...
  console.log('Launching browser to isolate the bottle and completely remove the floor shadow...');
  ...
  const result = await page.evaluate(async () => {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Cache row backgrounds (sample left and right edges)
        const rowBg = [];
        ...
        for (let y = 0; y < height; y++) {
          ...
          for (let x = 0; x < width; x++) {
            ...
            // Use stricter threshold at the bottom (y > 1400) to ignore shadow for bounds
            const isBottom = y > 1400;
            const thresh = isBottom ? 55 : 35;
        ...
        // Loop over the destination canvas and copy pixels with clean feathering
        for (let dy = 0; dy < 800; dy++) {
          for (let dx = 0; dx < 800; dx++) {
            ...
            if (x >= minX && x <= maxX && y >= minY && y <= maxY) {
              ...
              // Use dynamic threshold: Stricter at the bottom to aggressively cut off shadow
              // Base of the bottle is around Y=1450. The glass is very dark (dist > 80).
              let minThreshold = 28;
              let maxThreshold = 38;

              if (y > 1400) {
                // Ramp up the threshold from Y=1400 to Y=1550
                const bottomFactor = Math.min(1.0, (y - 1400) / 150);
                minThreshold = 28 + (40 * bottomFactor); // Goes up to 68
                maxThreshold = 38 + (40 * bottomFactor); // Goes up to 78
              }
  ...
  console.log(`Successfully wrote perfectly clean Moringa Oil image to: ${destPath}`);
