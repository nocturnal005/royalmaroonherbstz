...
async function fixOne() {
  const file = "ZOZ_2226.JPG";
  const sourcePath = path.join("D:\\new product listings\\Highlights", file);
  const destPath = path.join(path.resolve("public/images/products/royal-herbs"), file);
  ...
  const result = await page.evaluate(async (uri) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        // STEP 4: DO NOT ROTATE! The wide aspect is just shadow.
  ...
  fs.writeFileSync(destPath, Buffer.from(base64Bytes, 'base64'));
  await browser.close();
  console.log("Fixed 105");
}
