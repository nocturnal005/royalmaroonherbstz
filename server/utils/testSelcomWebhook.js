...
// money moves. With PAYMENTS_MODE=mock it creates real checkout sessions +
// orders, calls POST /api/payments/initiate (mobile money USSD flow and the
// mobile money USSD flow), then simulates Selcom's webhook delivery
...
// order_id, amount mismatch, and failure result.
...
async function runTests() {
  ...
  if (process.env.PAYMENTS_MODE !== 'mock') {
    console.log('PAYMENTS_MODE must be "mock" to run this harness (Selcom has no sandbox). Aborting.');
    process.exit(1);
  }
