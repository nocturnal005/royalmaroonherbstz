...
/**
 * Full webhook URL including its secret path segment. Sent per-order in the
 * create-order `webhook` field, so no dashboard configuration is needed.
 */
function buildSecretWebhookUrl() {
 ...
 * Initiates a Selcom mobile money payment (mpesa/tigo/airtel) via
 * create-order-minimal + wallet-payment USSD push, with status polling.
 * Protected by idempotency.
 *
 * Card payments are deliberately not supported: Selcom offers mobile money
 * only, so the checkout no longer presents a card option.
 */
