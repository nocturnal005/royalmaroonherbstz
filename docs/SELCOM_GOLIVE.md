...
(30 checks: mobile money + card initiate, webhook success/failure/idempotency/
bad-secret/disallowed-IP/unknown-reference/amount-mismatch, single stock
deduction) runs the whole flow with `SELCOM_MODE=mock`. These are the steps to
switch to live.
...
```
SELCOM_MODE=                                  # unset/anything but "mock" = real calls
